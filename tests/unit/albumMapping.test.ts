...
describe('should Tag to ablum mapping', () => {
  ...
  test('should throw error if tag ID format is invalid', async () => {
    await expect(
      albumMap.addMapping(invalidTagId, validAlbumId)
    ).rejects.toThrow('Invalid tag ID format');
  });

  test('should throw error if album ID format is invalid', async () => {
    await expect(
      albumMap.addMapping(validTagId, invalidAlbumId)
    ).rejects.toThrow('Invalid Spotify album ID format');
  });
  ...
  test('should throw error when updating with invalid tag ID format', async () => {
    await albumMap.addMapping(tagId, albumId);
    await expect(
      albumMap.updateMapping(invalidTagId, newAlbumId)
    ).rejects.toThrow('Invalid tag ID format');
  });

  test('should throw error when updating with invalid album ID format', async () => {
    await albumMap.addMapping(tagId, albumId);
    await expect(albumMap.updateMapping(tagId, invalidAlbumId)).rejects.toThrow(
      'Invalid Spotify album ID format'
    );
  });
});
